...
class Evaluator:
    def __init__(self, dataset='lifeguard'):
        ...
        for c in classes:
            ...
            for d in range(len(dects)):
                ...
                # - A detected "cat" is overlaped with a GT "cat" with IOU >= IOUThreshold.
                    ...
                    FP[d] = 1  # count as false positive
                    # print("FP")
            # by calculating the accumulated FP and TP values we get precision-recall-pairs
            # at certain confidence threshold-levels. we do not care about the confidence values,
            # because we just need the precision-recall curve to calculate the mAP. we needed to sort by confidence
            # level to be able to calculate the accumulated value
            acc_FP = np.cumsum(FP)
            acc_TP = np.cumsum(TP)
            # reminder: npos was calculated earlier, but theoretically if you sum up all ones from FP and TP, that would be npos. as the confidence is coupled to the output of neural network
            rec = acc_TP / npos
    ...
    def PlotPrecisionRecallCurve(self,
        ...
        for result in results:
            ...
            if savePath is not None:
                ...
                # save fig
                plt.savefig(os.path.join(savePath, weight_name + '_' + classId + '.png'))
...
def getGTBoundingBoxes(allBoundingBoxes, allClasses, keyframe_indices):
    for snippet, keyframe in keyframe_indices:
        box_and_label_list = FileLoader.get_box_and_label_list(snippet, keyframe)

        for xtl, ytl, xbr, ybr, label in box_and_label_list:
            nameOfImage = f'{snippet}_{keyframe}'
            idClass = str(label)
            bb = BoundingBox(
                nameOfImage,
                idClass,
...
def getDetBoundingBoxes(allBoundingBoxes, allClasses, det_boxes):
    for snippet, keyframe, xtl, ytl, xbr, ybr, cls_id, score in det_boxes:
        nameOfImage = f'{snippet}_{keyframe}'
        idClass = str(cls_id)
        bb = BoundingBox(
            nameOfImage,
...
def evaluate_frameAP(keyframe_indices, det_boxes, weight_name, threshold = 0.5, savePath = None, show_pr_curve=False):
    ...
    allBoundingBoxes, allClasses = getGTBoundingBoxes(
        allBoundingBoxes, allClasses, keyframe_indices)
    # Get detected boxes
    allBoundingBoxes, allClasses = getDetBoundingBoxes(
        allBoundingBoxes, allClasses, det_boxes)
    allClasses.sort()
