...
class Lifeguard_Evaluator_3Dseq(object):
    def __init__(self,
                 d_cfg,
                 iou_thresh=0.5,
                 collate_fn=None):
        ...
        # CHANGES ########################################################################
        # dataset
        self.testset = LifeguardDataset_3Dseq(
            cfg=d_cfg,
            mode='test')
    ...
    def evaluate_frame_map(self, model, epoch=1, show_pr_curve=False):
        ...
        for iter_i, (batch_frame_id, batch_video_clip, batch_target) in enumerate(self.testloader):
                # CHANGES ########################################################################
                batch_frame_id=batch_frame_id[0] # eliminate sequence dimension S
                batch_target=batch_target[0] # eliminate sequence dimension S because we have sequence length 1 for test, keep batch dimension because for original problem we also kept it for target
