...
class YOWO_2D3Dseq(nn.Module):
    def __init__(self, 
                 ...
                 trainable = False,
                 multi_hot = False):
        super(YOWO_2D3Dseq, self).__init__()
        self.cfg = cfg
        self.d_cfg = d_cfg
        self.device = device
        ...
        ## convlstm
        self.convlstm = ConvLSTM(464, 464, (3,3), d_cfg['layers'], return_all_layers=True)
    ...
    @torch.no_grad()
    def inference(self, video_clips_seq, hidden_state):
        """
        Input:
            video_clips: (Tensor) -> [B, 3, T, H, W].
        return:
        """
        B, _, _, img_h, img_w = video_clips_seq[0].shape


        key_frame_seq, feat_3Dseq, cls_feats_seq, reg_feats_seq = [], [], [], []
        for video_clips in video_clips_seq:
            # key frame
            key_frame = video_clips[:, :, -1, :, :]
            key_frame_seq.append(key_frame)
            # 3D backbone
        ...
        feat_3d, cls_feat, reg_feat = feat_3d_output[0], cls_feats_seq[0], reg_feats_seq[0]
        ...
        if self.multi_hot:
            ...
            return batch_scores, batch_labels, batch_bboxes, hidden_state


    def forward(self, video_clips_seq, hidden_state = None):
        """
        Input:
            old: video_clips: (Tensor) -> [B, 3, T, H, W].
            new: frame_seq: (Tensor) ->   [S, B, 3, T, H, W].
        return:
            outputs: (Dict) -> {
        ...
        if not self.trainable:
            return self.inference(video_clips_seq, hidden_state)
        else:
            key_frame_seq, feat_3Dseq, cls_feats_seq, reg_feats_seq = [], [], [], []
            for video_clips in video_clips_seq:
                # key frame
            ...
            outputs_seq=[]
            for feat_3d, cls_feats, reg_feats in zip(feat_3d_output, cls_feats_seq, reg_feats_seq):
