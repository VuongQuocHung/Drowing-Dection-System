...
class YOWO_2Dseq(nn.Module):
    def __init__(self, 
                 ...
                 trainable = False,
                 multi_hot = False):
        super(YOWO_2Dseq, self).__init__()
        self.cfg = cfg
        self.d_cfg = d_cfg
        self.device = device
        ...
        ## convlstm: tiny: 64,64, medium: 256, 128
        version = d_cfg['version']
        if version == 'yowo_v2_nano' or version == 'yowo_v2_tiny':
            in_ch = 64
            out_ch = 64
        elif version == 'yowo_v2_medium':
            in_ch = 256
            out_ch = 128
        else:
            raise NotImplementedError
        self.convlstm_cls = nn.ModuleList([ConvLSTM(in_ch, out_ch, (3,3), d_cfg['layers'], return_all_layers=True), ConvLSTM(in_ch, out_ch, (3,3), d_cfg['layers'], return_all_layers=True), ConvLSTM(in_ch, out_ch, (3,3), d_cfg['layers'], return_all_layers=True)])
        self.convlstm_reg = nn.ModuleList([ConvLSTM(in_ch, out_ch, (3,3), d_cfg['layers'], return_all_layers=True), ConvLSTM(in_ch, out_ch, (3,3), d_cfg['layers'], return_all_layers=True), ConvLSTM(in_ch, out_ch, (3,3), d_cfg['layers'], return_all_layers=True)])
    ...
    @torch.no_grad()
    def inference(self, frame_seq, hidden_state):
        """
        Input:
        old: video_clips: (Tensor) -> [B, 3, T, H, W].
        new: frame_seq: (Tensor) ->   [S, B, 3, H, W].
        return:
        """
        _, B, _, img_h, img_w = frame_seq.shape


        cls_feats_seq, reg_feats_seq = [], []
        for frame in frame_seq:
            # # 3D backbone
            # feat_3d = self.backbone_3d(video_clips)
            # feat_3Dseq.append(feat_3d)
            # 2D backbone          input-format [B, 3, H, W]  (instead earlier extract at last T position, now iterate over S)
            cls_feats, reg_feats = self.backbone_2d(frame)
            cls_feats_seq.append(cls_feats)
            reg_feats_seq.append(reg_feats)
        
        # feat_3Dseq = torch.stack(feat_3Dseq)
        # layer_output_list, hidden_state = self.convlstm(feat_3Dseq, hidden_state)
        # feat_3d_output = layer_output_list[0].permute(1, 0, 2, 3, 4)
        # feat_3d_output = [f for f in feat_3d_output]


        new_hidden_state = [[None,None,None], [None,None,None]]
        for l in range(3):      

            cls_feats_seq_level_1 = [f[l] for f in cls_feats_seq]
            reg_feats_seq_level_1 = [f[l] for f in reg_feats_seq]
            cls_feats_seq_level_2 = torch.stack(cls_feats_seq_level_1)
            reg_feats_seq_level_2 = torch.stack(reg_feats_seq_level_1)

            cls_feats_seq_level_3, new_hidden_state[0][l] = self.convlstm_cls[l](cls_feats_seq_level_2, hidden_state[0][l])
            reg_feats_seq_level_3, new_hidden_state[1][l] = self.convlstm_reg[l](reg_feats_seq_level_2, hidden_state[1][l]) 
            cls_feats_seq_level_4 = cls_feats_seq_level_3[0].permute(1, 0, 2, 3, 4)
            reg_feats_seq_level_4 = reg_feats_seq_level_3[0].permute(1, 0, 2, 3, 4)
            cls_feats_seq_level_5 = [f for f in cls_feats_seq_level_4]
            reg_feats_seq_level_5 = [f for f in reg_feats_seq_level_4]

            for i in range(len(cls_feats_seq)):
                cls_feats_seq[i][l] = cls_feats_seq_level_5[i]
                reg_feats_seq[i][l] = reg_feats_seq_level_5[i]

    

        # the previous part we just copied from forward function to be consistent. now because of different return no iteration
        cls_feat, reg_feat = cls_feats_seq[-1], reg_feats_seq[-1]
        ...
        if self.multi_hot:
            batch_bboxes = []
            for batch_idx in range(frame.size(0)):
                cur_conf_preds = []
                cur_cls_preds = []
                cur_reg_preds = []
            ...
            return batch_scores, batch_labels, batch_bboxes, new_hidden_state


    def forward(self, frame_seq, hidden_state = [[None,None,None], [None,None,None]]):   
        """
        Input:
        old: video_clips: (Tensor) -> [B, 3, T, H, W].
        new: frame_seq: (Tensor) ->   [S, B, 3, H, W].
        ...
        if not self.trainable:
            return self.inference(frame_seq, hidden_state)
        else:
            cls_feats_seq, reg_feats_seq = [], []
            for frame in frame_seq:
                # # 3D backbone
            ...
            outputs_seq=[]
            for cls_feats, reg_feats in zip(cls_feats_seq, reg_feats_seq):
