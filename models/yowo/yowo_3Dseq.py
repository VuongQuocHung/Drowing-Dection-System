...
class YOWO_3Dseq(nn.Module):
    def __init__(self, 
                 ...
                 trainable = False,
                 multi_hot = False):
        super(YOWO_3Dseq, self).__init__()
        self.cfg = cfg
        self.d_cfg = d_cfg
        self.device = device
        ...
        ## convlstm
        self.convlstm = ConvLSTM(464, 64, (3,3), d_cfg['layers'], return_all_layers=True)
    ...
    def inference(self, video_clips_seq, hidden_state):
        ...
        key_frame_seq, feat_3d_seq= [], []
        for video_clips in video_clips_seq:
            # 3D backbone
            feat_3d = self.backbone_3d(video_clips)
            feat_3d_seq.append(feat_3d)
        feat_3d_seq = torch.stack(feat_3d_seq)

        layer_output_list, hidden_state = self.convlstm(feat_3d_seq, hidden_state)
        ...
        for level in range(2):
            ...
            # head
            cls_feat, reg_feat = self.heads[level](feat_3d_up, feat_3d_up)
